...
author: "Rara dwi davisca A3202300036 & Muhammad Ihsan Najih Amani A3202300031"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
## 4.1 Dataset A: Prediksi Harga Barang
Dataset buatan untuk prediksi harga barang dengan variabel:
- `kualitas`: Kualitas produk (1-10)
- `berat`: Berat produk (gram)
- `brand_score`: Skor brand (1-5)
- `umur_produk`: Umur produk (bulan)
- `harga`: Harga produk (rupiah) - variabel target

## 4.2 Dataset B: Formula 1 Performance
Dataset untuk analisis performa Formula 1 dengan variabel:
- `engine_power`: Kekuatan mesin (HP)
- `aerodynamics`: Skor aerodinamika (1-100)
- `tire_quality`: Kualitas ban (1-10)
- `driver_skill`: Skill driver (1-100)
- `lap_time`: Waktu lap (detik) - variabel target

# 5. Teknologi dan Library yang Digunakan
